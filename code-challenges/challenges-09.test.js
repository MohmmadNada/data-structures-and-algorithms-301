...
function transformToLis(obj) {
    ...
    for (let keys in obj) {
        ...
        element.textContent(`${keys}: ${obj[keys]}`)
        arr.push(element);
        return arr;
    }
    return arr;
...
const addPurchases = (arr) => {
    // Solution code here...
    return arr.reduce((acc, value, index) => acc = acc + value.purchasePrice, 0)
};
...
const countNumberOfElements = (arr) => {
    // Solution code here...
    return arr.reduce((acc, element, index) => acc = acc + 1, 0)
};
