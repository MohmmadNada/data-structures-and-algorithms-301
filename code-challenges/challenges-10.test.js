...
const getNames = (arr) => {
    let arrayNew = arr.map(element => {
        let edit = element.name.split('').reverse().join('')
            // console.log(edit)
...
const count = (target, input) => {
    // Solution code here...
    let countTarget = 0;
    let newArr = input.map(items => {
        let innerArr = items.filter(itemsInner => itemsInner == target)
        countTarget += innerArr.length;
    })
    return countTarget;
};
...
const totalSum = (input) => {
    // Solution code here...
    let sum = 0;
    input.map(externalItem => {
        return externalItem.map(internalItem => sum += internalItem);
    })
    return sum;
};
...
const divisibleByFiveTwoToThePower = (input) => {
    // remove any elements that are not numbers and not divisible by five
    let arr1 = input.map(element => {
        let arr2 = element.filter((element) => {
            if (typeof element === 'number' && element % 5 == 0) {
                return true;
            }
        })
        let arr3 = arr2.map(element1 => {
            return Math.pow(2, element1);
        })
        return arr3;
    })
    return arr1;
...
let findMaleAndFemale = (data) => {
    // Solution code here...
    // returnnnnnnnnnnnnn 
    let arrFilter = data.filter(element => {
        if (element.gender == 'female' || element.gender == 'male') {
            return true;
        }
    });

    let arrResu = arrFilter.reduce((acc, val, idx) => {
        acc[idx] = val.name;
        return acc;
    }, []);

    return arrResu.join(' and ');
};
